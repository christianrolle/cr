# Presents articles in the view layer
class ArticlePresenter < Presenter
  def tags_with_icon
    ...
    h.content_tag(:div, class: css) do
      article_tag_positions.inject(''.html_safe) do |html, article_tag_position|
  ...
  def find_article_relation(kind)
    model.article_relations.detect { |relation| relation.kind == kind }
  end
